...
export function ShareButton({ hangoutId }: ShareButtonProps) {
    ...
    const handleShare = async () => {
        ...
        if (!urlToShare) {
            setIsLoading(true);
            try {
                const res = await fetch(`/api/hangouts/${hangoutId}/invite`, {
                    method: "POST"
                });
            ...
            if (navigator.share) {
                try {
                    await navigator.share({
                        title: 'You invited!',
                        text: 'Join my hangout on Plans. See the itinerary & RSVP via the link.',
                        url: urlToShare,
                    });
                } catch (err) {
                    // Fallback (e.g. if user cancels share sheet)
